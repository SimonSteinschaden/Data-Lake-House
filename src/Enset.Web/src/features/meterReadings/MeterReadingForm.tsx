...
export function MeterReadingForm({initial,entityId,unit,onClose,onSaved}:{initial:MeterReadingWriteModel;entityId?:string;unit:string;onClose:()=>void;onSaved:()=>void}){
 ...
 <FormField label="Messwerttyp" error={errors.readingType}><input required value={model.readingType} onChange={e=>set("readingType",e.target.value)}/></FormField>
 <FormField label="Qualität" error={errors.qualityFlag}><input required value={model.qualityFlag} onChange={e=>set("qualityFlag",e.target.value)}/></FormField>
