...
export function DataProductsPage(){
 ...
 return <main><h1>Datenprodukte</h1><p>Versionierte, reproduzierbare Ausgabemodelle auf Basis kanonischer Datenstände.</p><div className="catalog-filters"><input aria-label="Datenprodukte suchen" placeholder="Suchen …" value={search} onChange={e=>setSearch(e.target.value)}/><select aria-label="Kategorie filtern" value={category} onChange={e=>setCategory(e.target.value)}><option value="">Alle Kategorien</option>{categories.map(x=><option key={x}>{x}</option>)}</select></div>{error&&<p className="error">{error}</p>}<div className="product-grid">{visible.map(({metadata:p,lastUpdatedUtc})=><Link className="product-card" to={`/data-products/${p.code}`} key={p.code}><small>{p.category}</small><h2>{p.germanName}</h2><p>{p.description}</p><dl><dt>Version</dt><dd>{p.version.major}.{p.version.minor}.{p.version.patch}</dd><dt>Qualität</dt><dd>{p.qualityLevel}</dd><dt>Eignung</dt><dd>{p.suitability}</dd><dt>Aktualisiert</dt><dd>{new Date(lastUpdatedUtc).toLocaleString("de-AT")}</dd></dl></Link>)}</div></main>
}
